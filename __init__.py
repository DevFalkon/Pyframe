...
import os
from Pyframe.widgets import Circle
...
class GuiWindow:
    def __init__(self, width=1020, height=720, title="PygameGUI", logo=None):
        ...
        # variables for manual resizing of the app window
    ...
    def win_update(self):
        ...
        if self.close_button.circle_dist(mouse_pos) and not self.drag_lock:

            if left_click:
                self.quit_app()

        elif self.min_button.circle_dist(mouse_pos) and left_click and not self.drag_lock:
            pg.display.iconify()

        elif self.max_button.circle_dist(mouse_pos) and not self.drag_lock:
            self.drag_lock = True
            if left_click:
        ...
        if left_click and mouse_pos[1] > self.top_bar_height:
            if self.drag_lock and self.minimise:
        ...
        if mouse_x <= border or self.left_resize_lock:
            if not (self.right_resize_lock or self.bottom_resize_lock):
                if not self.left_resize_lock:
                    ...
                    if final_pos != self.initial_mouse_position:
    ...
    def title_bar(self, color='grey', font_color='white'):
        title_bar_color = colors[color]
        pg.draw.rect(self.screen, title_bar_color, pg.Rect(0, 0, self.width, self.top_bar_height))

        if self.title:
            title_size = self.top_bar_height - 15
            font = pg.font.Font(f'{os.getcwd()}\\Pyframe\\fonts\\FiraMono-Medium.ttf', title_size)
            text = font.render(self.title, True, colors[font_color])
        ...
        self.close_button = Circle(self.screen, self.width - 25,
                                   self.top_bar_height // 2,
                                   self.top_bar_height // 4,
                                   colors['red'])
        self.max_button = Circle(self.screen, self.width - 50,
                                 self.top_bar_height // 2,
                                 self.top_bar_height // 4,
                                 colors['yellow'])
        self.min_button = Circle(self.screen, self.width - 75,
                                 self.top_bar_height // 2,
                                 self.top_bar_height // 4,
                                 colors['green'])
